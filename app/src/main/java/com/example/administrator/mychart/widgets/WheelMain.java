package com.example.administrator.mychart.widgets;
...
public class WheelMain {
	...
	/**
	 * @Description: TODO 弹出日时选择器
	 */
	public void initDateTimePicker2(int year, int month, int day, int h, int m) {
		// 添加大小月月份并将其转换为list,方便之后的判断
		String[] months_big = { "1", "3", "5", "7", "8", "10", "12" };
		String[] months_little = { "4", "6", "9", "11" };
		...
		OnWheelChangedListener wheelListener_year = new OnWheelChangedListener() {
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				int year_num = newValue + START_YEAR;
				// 判断大小月及是否闰年,用来确定"日"的数据
					wv_day.setAdapter(new NumericWheelAdapter(0, 100));
		...
		if (iTime == 2) {
			...
			// 判断大小月及是否闰年,用来确定"日"的数据
			if (list_big.contains(String.valueOf(month + 1))) {
				wv_day.setAdapter(new NumericWheelAdapter(0, 31));
			} else if (list_little.contains(String.valueOf(month + 1))) {
				wv_day.setAdapter(new NumericWheelAdapter(0, 30));
			} else {
				// 闰年
				if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)
					wv_day.setAdapter(new NumericWheelAdapter(0, 29));
				else
					wv_day.setAdapter(new NumericWheelAdapter(0, 28));
			}
			wv_day.setLabel("日");
			wv_day.setCurrentItem(day);
	...
	public String getTime() {
		...
		if (iTime == 1) {
			...
			if (wv_hours.getCurrentItem() < 10) {
				sb.append("0" + wv_hours.getCurrentItem());
			} else {
	...
	public int getTime2() {
		...
		value = (wv_day.getCurrentItem()) * 24;
		if (wv_hours.getCurrentItem() < 10) {
			value += 0 + wv_hours.getCurrentItem();
		} else {
			value += wv_hours.getCurrentItem();
		}
